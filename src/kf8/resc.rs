...
mod tests {
    ...
    #[test]
    fn encode_allocates_all_required_payload_blocks() {
        let sections = (0..64)
            .map(|index| KindleSection {
                id: format!("section-{index}-{}", "x".repeat(96)),
                href: format!("section-{index}.xhtml"),
                source_xhtml: String::new(),
                referenced_styles: Vec::new(),
                linear: true,
                layout: Default::default(),
                rendition: Default::default(),
                source_properties: vec!["rendition:page-spread-left".to_owned()],
                source_spine_index: Some(index),
            })
            .collect::<Vec<_>>();
        let record = encode(&sections, RenditionSemantics::default(), None)
            .expect("encode multi-block RESC");
        let header_length = u32::from_be_bytes(record[12..16].try_into().unwrap()) as usize;
        let payload = &record[RESC_PREFIX_SIZE..];
        let header = std::str::from_utf8(&payload[..header_length]).unwrap();
        let xml_length = header
            .strip_prefix("size=")
            .and_then(|value| value.split_once('&'))
            .map(|(value, _)| value)
            .map(|value| {
                value.bytes().fold(0usize, |total, digit| {
            ...
            .unwrap();
        let payload_length = header_length + xml_length;
        let payload_blocks = payload_length.div_ceil(RESC_PAYLOAD_SIZE);
        assert!(payload_blocks > 1);
        assert_eq!(
            record.len(),
            RESC_PREFIX_SIZE + payload_blocks * RESC_PAYLOAD_SIZE
        );
        assert!(payload[payload_length..].iter().all(|byte| *byte == 0));
    }
}
